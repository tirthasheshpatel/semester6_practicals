    ...
    Graph(std::set<int> _V, std::set<std::pair<int, int> > _E)
    {
        for ( auto it = _E.begin() ; it != _E.end() ; ++it ) {
            ...
            if ( _V.find(v1) == _V.end() || _V.find(v2) == _V.end() ) {
                throw std::logic_error("error: The edge set contains vertices not present in the vertex set!");
            }
        }
...
private:
    ...
    template <typename T>
    std::set<T> __get_union(const std::set<T> &a, const std::set<T> &b)
    {
        std::set<T> result = a;
    ...
    template <typename T>
    std::set<T> __get_intersection(const std::set<T>& a, const std::set<T>& b)
    {
    ...
    template <typename T>
    std::set<T> __get_difference(const std::set<T>& a, const std::set<T>& b)
    {
        std::set<T> result;
...
int main()
{
    ...
    std::cout << std::endl;
    std::cout << "Union of the G1 and G2 is : " << std::endl;
    std::cout << "G3:\n";
    std::cout << G3 << std::endl;
    std::cout << "Intersection of the G1 and G2 is : " << std::endl;
    std::cout << "G4:\n";
    std::cout << G4 << std::endl;
    std::cout << "Ring Sum of the G1 and G2 is : " << std::endl;
    std::cout << "G5:\n";
